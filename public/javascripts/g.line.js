...
Raphael.fn.g.linechart = function (x, y, width, height, valuesx, valuesy, opts) {
    ...
    var cx = x, cy = y, cw = width, paper = this, series = this.set();
    var legend = function (labels, otherslabel, mark, dir) {
        ...
        for (var i = 0, ii = labels.length; i < ii; i++) {
            var clr = colors[i],//series[i].attr("fill"),
                j = i,//values[i].order,
                txt;
            labels[j] || (labels[j] = otherslabel || "Others");
            //labels[j] = paper.g.labelise(labels[j], i, 100);
            ...
            //series[i].label = chart.labels[i];
            h += txt.getBBox().height * 1.2;
        }
        var bb = chart.labels.getBBox(),
            tr = {
                east: [0, -bb.height / 2 + 30],
                west: [-bb.width - cw - 20, -bb.height / 2],
                north: [-cw - bb.width / 2, -cw - bb.height - 10],
                south: [-cw - bb.width / 2, cw + 10]
            }[dir];
